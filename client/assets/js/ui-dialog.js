...
function prompt(message, dialogContainerClass, textContainerClass, buttonsToDisplay, extraDivToDisplay) {
    ...
    let promptContainerDiv = $('<div>', {
        class: 'gole-prompt-container'
    });
    openPromptContainer = promptContainerDiv;

    let promptDialogContainerDiv = $('<div>', {
        class: 'gole-prompt-dialog-container ' + dialogContainerClass
    });
    ...
    let promptDialogTextContainer = $('<div>', {
...
function promptError(message, acceptButtonText, onAcceptCallback) {
    ...
    let callbackArguments = Array.prototype.slice.call(arguments, 3);

    let buttonsToDisplay = new Array();
    let acceptButton = $('<button>', {
        class: 'gole-prompt-dialog-button gole-prompt-dialog-neutral-button'
    });
...
function promptGameOver(scoreboard) {
    ...
    $.each(scoreboard, function(playerName, playerPoints) {
        if ((winnerPlayerNames == null) || (playerPoints > scoreboard[winnerPlayerNames[0]])) {
            console.log("set winner player names to array")
            winnerPlayerNames = [];
    ...
    let newGameButton = $('<button>', {
        class: 'gole-prompt-dialog-button gole-prompt-dialog-success-button'
    });
...
function promptWildcardLetterSelection(onSelectCallback) {
    ...
    let extraCallbackArguments = Array.prototype.slice.call(arguments, 1);
    let alphabeth_en = 'abcdefghijklmnopqrstuvwxyz'.split('');

    let alphabethContainerDiv = $('<div>');

    $.each(alphabeth_en, function(_, letterCharacter) {
        let tileDiv = $('<div>', {
            class: 'gole-tile gole-tile-selectable gole-tile-margin'
        });
        let letterDiv = $('<div>', {
            class: 'gole-tile-letter-character-container'
        });
        ...
        tileDiv.click(function() {
            ...
            let callbackArguments = [letterCharacter.charCodeAt()].concat(extraCallbackArguments);
    ...
    let dismissButton = $('<button>', {
        class: 'gole-prompt-dialog-button'
    });
...
function promptNewGame(onStartCallback) {
    ...
    let callbackArguments = Array.prototype.slice.call(arguments, 1);
    let nameTextFieldsContainer = $('<div>');

    TEXT_FIELDS_TO_DISPLAY = 4;
    let nameTextFields = new Array();
    for (let inputTextFieldCounter = 0; inputTextFieldCounter < TEXT_FIELDS_TO_DISPLAY; inputTextFieldCounter++) {
        let nameTextField = $('<input>', {
            class: 'gole-prompt-dialog-text-field',
            type: 'text',
            placeholder: 'Player ' + (inputTextFieldCounter + 1)
        });
...
function playPointsGainAnimation(pointsGained, words) {
    ...
    let animationContainerDiv = $('<div>', {
        class: 'points-gained-animation-container'
    });
    let pointsContainer = $('<div>', {
        class: 'points-gained-animation-points-container'
    });
