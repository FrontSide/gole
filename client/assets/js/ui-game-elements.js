...
function drawBoard() {
    ...
    $.each(game.board, function(yIdx, column) {
        $.each(column, function(xIdx, tile) {
            ...
            let boardTileDiv = $('<div>', {
                class: 'gole-board-tile ' + tileEffectColorClass
            });
            ...
            if (tile.Letter.Character == 0 && tileEffectText) {
                ...
                var tileDiv = $('<div>', {
                    class: 'gole-tile gole-tile-margin'
                });
                let letterDiv = $('<div>', {
                ...
                let letterValueDiv = $('<div>', {
                    class: 'gole-tile-letter-value-container'
                });
                letterValueDiv.html(tile.Letter.Attributes.PointValue);

                let potentialPointsDiv = $('<div>', {
                    class: 'gole-tile-potential-points-appendix-container'
                });
                ...
                if (!tile.IsLocked) {
                    ...
                    tileDiv.on('dragstart', function() {
                        if (activatedLetter != tile.Letter) {
                            ...
                            if (potentialPointsDiv != null) {
                                potentialPointsDiv.remove();
                            }
                            removeLetter();
                        }
                    })
...
// back to the hand.
let handContainerDiv = null;

function drawPlayer() {
    $('div.gole-active-player-container').html('');

    let nameDiv = $('<div>', {
        class: 'gole-active-player-name-container'
    });
    nameDiv.html(activePlayer.Name);

    let pointsDiv = $('<div>', {
        class: 'gole-active-player-points-container'
    });
    pointsDiv.html(activePlayer.Points + ' Points');

    handContainerDiv = $('<div>', {
        class: 'gole-active-player-hand-container'
    });
    handContainerDiv.sortable({
        ...
        receive: function(event, ui) {
            deactivateLetter();
            letterIdsinHand = getArrayOfLetterIdsInHand(handContainerDiv);
            $.when(rearrangeLettersInHand(letterIdsinHand)).done(function() {
                console.log('finished rearranging letters');
                reload();
            });
        },
    });
    ...
    let confirmWordButton = $('<button>', {
        class: 'gole-gameplay-button gole-confirm-word-button'
    });
    confirmWordButton.html('Confirm Word');
    confirmWordButton.click(function() {
        $.when(confirmWord())
            .then(function(responseBody, _, httpResponseObject) {
                    if (httpResponseObject.status == 200) {
                        wordConfirmSuccessRoutine(JSON.parse(responseBody).GainedPoints);
                    } else if (httpResponseObject.status == 250) {
                        gameOver();
                    }
                },
                function(httpResponseObject) {
                    wordConfirmErrorRoutine(httpResponseObject.responseText);
                }
            );
    });

    let shuffleHandButton = $('<button>', {
        class: 'gole-gameplay-button gole-shuffle-hand-button'
    });
    ...
    shuffleHandButton.click(function() {
        ...
        $.when(rearrangeLettersInHand()).done(reload);
    });
